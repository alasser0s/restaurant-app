    ...
    return (
      <div className="max-w-6xl mx-auto py-16 px-4 bg-white dark:bg-darkbg text-black dark:text-gray-200">
        ...
        >
          ...
          <p className="font-light text-black dark:text-gray-300">Your delicious food will be with you soon. Grazie!</p>
        </motion.div>
      </div>
    )
  }
  ...
  return (
    <div className="max-w-6xl mx-auto py-16 px-4 bg-white dark:bg-darkbg text-black dark:text-gray-200">
      ...
      {cartDetails.length === 0 ? (
        <p className="text-black dark:text-gray-300 font-light">
          Your cart is empty. Please add items from our <a href="/menu" className="text-gold hover:opacity-90">menu</a>.
        </p>
      ) : (
        <div className="max-w-md mx-auto bg-gray-100 dark:bg-gray-900/60 backdrop-blur-sm rounded p-6 border border-gray-200 dark:border-gray-800">
          <ul className="space-y-4 mb-6">
            {cartDetails.map(item => (
              <li key={item.id} className="flex justify-between items-center font-light">
                <span className="text-black dark:text-gray-200">
                  {item.name} <span className="text-gray-600 dark:text-gray-400">x {item.quantity}</span>
                </span>
                <span className="text-black dark:text-gray-200">${item.price * item.quantity}</span>
              </li>
            ))}
          </ul>
          <div className="flex justify-between items-center mb-6 pt-4 border-t border-gray-300 dark:border-gray-800">
            <span className="font-semibold text-black dark:text-gray-200">Total</span>
            <span className="font-semibold text-black dark:text-gray-200">${total}</span>
          </div>
          <button 
            onClick={handleOrder} 
            className="w-full bg-gold text-black dark:text-gray-900 py-2 px-4 rounded font-semibold hover:opacity-90 transition">
            Confirm Order
          </button>
        </div>
      )}
